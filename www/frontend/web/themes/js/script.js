...
function initMap() {
    ...
    var marker = new google.maps.Maprker({
        position: uluru,
        map: map
    });
};
var li;
$('.edit-menu li a').each(function(i){
    var hrefa = $(this).attr('href');
    if(hrefa == window.location.href) {
        li = i;
    }
});
$('.edit-menu li').each(function(index){
    if (li == index) {
...
$(function() {
    function slideMenu() {
        var activeState = $(".menu-list-mobile").hasClass("active");
        $(".menu-list-mobile").animate({left: activeState ? "0%" : "-110%"}, 400);
    }
  // $("#menu-wrapper-mobile").click(function(event) {
  //   event.stopPropagation();
  //   $(".menu-container-mobile .menu-list-mobile").toggleClass("active");
  //   slideMenu();
  // });
  var hide = true;
  $('body').on("click", function () {
    if (hide) $('.menu-list-mobile').removeClass('active');
    hide = true;
    slideMenu();
});
  $('body').on('click', '#menu-wrapper-mobile', function () {
    if ($('.menu-list-mobile').hasClass('active')) {
